...
//        }
        ...
        return QuizResource::collection($items)->additional([
            'success' => true,
            'message' => null,
            'errors' => null,
            'status'  => 200
...
//            return apiResponse(false, null, 'Version Not Identical.', null, 400);
//        }
        $item = Quiz::where('is_exam', 1)
                    ->with('classroom')
                    ->with('teacher')
                    ->with('subject')
                    ->find($id);
        if (!$item) {
            return apiResponse(false, null, null, __('api.not_found'), 404);
        }
        ...
        $questions    = Question::with('options')->whereIn('id', $questionsIds)->inRandomOrder()->get();
...
//        $item['my_answers'] = $myAnswers;
        ...
        //$item['answers_marks'] = $quizResult->marks ?? 0;
        $item['questions']  = $questions;
        //$item['my_answers'] = $myAnswers;
        return new QuizResource($item);
    }
...
//        }
        ...
        $item = Quiz::find($id);
        if (!$item) {
            return apiResponse(false, null, __('api.not_found'), null, 400);
        }
        $userId = auth()->id();
...
//        }
        ...
        $userId = auth()->id();
        $quizResult = QuizResult::where('in_progress', 2)->where('user_id', $userId)->where('quiz_id', $id)->first();
        if ($quizResult) {
        ...
        $quizResult->marks = $request->marks;
        $quizResult->in_progress = 2;
        $quizResult->save();
        return apiResponse(true, null, null, null, 200);
    }
}
